...
// Parse URL parameter for track outside DOMContentLoaded to guarantee early assignment
const urlParams = new URLSearchParams(window.location.search);
let trackParam = urlParams.get('track');

if (!trackParam) {
  const path = window.location.pathname.toLowerCase();
  if (path.includes('python.html')) {
    trackParam = 'python';
  } else if (path.includes('uiux.html')) {
    trackParam = 'uiux';
  } else {
    trackParam = 'data';
  }
...
if (trackParam === 'python') {
  currentModuleData = PythonData;
} else if (trackParam === 'uiux') {
  currentModuleData = UiUxData;
} else if (trackParam !== 'data') {
  currentModuleData = [{
    id: "placeholder_01",
    title: "Segera Hadir",
    icon: "🚧",
    description: "Modul ini sedang dalam tahap pengembangan.",
    theory: `<h3>Track Baru</h3>
             <p>Maaf, materi untuk track ini sedang disusun oleh tim kurikulum kami.</p>
             <p>Silakan kembali lagi nanti atau coba <strong>Data Analyst Track</strong> dan <strong>Web Developer Track</strong> yang sudah tersedia penuh!</p>`,
    quiz: {
      question: "Kapan modul ini akan tersedia?",
      options: ["Segera", "Tahun depan", "Tidak tahu", "Rahasia"],
      correctIndex: 0,
      topic: "Placeholder"
    }
  }];
